...
var socket = new _phoenix.Socket("/socket", {
  ...
  logger: function logger(kind, msg, data) {
    return console.log(kind + ": " + msg, data);
  }
});
...
var Video = {
  init: function init(socket, element) {
    ...
    _player2.default.init(element.id, playerId, function () {
      return _this.onReady(videoId, socket);
    });
  },
  onReady: function onReady(videoId, socket) {
    ...
    vidChannel.on("new_annotation", function (resp) {
      vidChannel.params.last_seed_id = resp.id;
    ...
    vidChannel.join().receive("ok", function (_ref) {
      ...
      var ids = annotations.map(function (ann) {
        return ann.id;
      });
      if (ids.length > 0) {
        vidChannel.params.last_seed_id = Math.max.apply(Math, _toConsumableArray(ids));
      }
      _this2.scheduleMessages(msgContainer, annotations);
  ...
  scheduleMessages: function scheduleMessages(msgContainer, annotations) {
    ...
    setTimeout(function () {
      ...
      var remainig = _this3.renderAtTime(annotations, ctime, msgContainer);
      _this3.scheduleMessages(msgContainer, remainig);
    }, 1000);
  },
